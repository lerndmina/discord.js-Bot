...
import { SlashCommandBuilder, EmbedBuilder, ApplicationCommand, AutocompleteInteraction } from "discord.js";
...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option.setName("timezone").setDescription("Your timezone").setRequired(true).setAutocomplete(true)
  );
...
export async function run({ interaction, client, handler }: SlashCommandProps) {
  ...
  const query = UserTimezone.default.findOne({ userId });
  const response = await query.exec();
  if(response) {
    response.timezone = timezone;
    await response.save();
  } else {
    const newUser = new UserTimezone.default({ userId, timezone });
    await newUser.save();
  }
